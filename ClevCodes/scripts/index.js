function opcoesNormais() {
	...
	searchBox.addEventListener('click', () => {
		...
		if (menu.classList.contains('inputshow')) {
			...
			listhBox.classList.remove('show')
		}
	})
	...
	const input = window.document.querySelector('.input-box input');
	const listhBox = window.document.querySelector('.listas-pacotes');

	input.addEventListener('click',()=>{
		if(input.value === ''){
			listhBox.classList.add('show')
		}
	});
